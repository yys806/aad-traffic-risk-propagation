...
from riskprop.sumo_e00 import SUMO_CELLS
...
def summarize(package: Path) -> dict[str, object]:
    ...
    report = json.loads((package / "e00_report.json").read_text(encoding="utf-8"))
    return {
...
def compare(primary: Path, clean: Path) -> dict[str, object]:
    ...
    equal = left == right
    return {
        "schema_version": "e00.environment-comparison.v1",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Compare two sealed E00 SUMO packages.")
