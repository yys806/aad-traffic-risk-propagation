"""Executable analytic and archived-path checks that precede E01 locking."""
...
from riskprop.sumo_e00 import analyze_physical_path_from_archived_tables
...
def write_e01_prelock_validation(output_dir: str | Path) -> Path:
    """Write preliminary E01 truth evidence while failing closed on final lock."""

    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=False)
    audit = {
        "schema_version": "e01.prelock-validation.v1",
        "experiment_id": "E01",
        ...
        "note": "Truth fixtures can validate implementations, but cannot set thresholds, delta_eq, delta_R, or E02 sample size.",
    }
    (output_dir / "audit.json").write_text(
        json.dumps(audit, ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    manifest = {
        "schema_version": "e01.prelock-manifest.v1",
        "files": [
            {
                "path": "audit.json",
