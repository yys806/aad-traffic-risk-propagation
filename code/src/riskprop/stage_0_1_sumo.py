"""Minimal real-SUMO Stage 0.1 runner and read-only analyzer.

This module deliberately implements only the Stage 0.1 infrastructure run.  It is
...
def _implementation_hashes(repo_root: Path) -> dict[str, str]:
    ...
    files = (
        ...
        code_root / "scripts" / "run_stage_0_1_sumo.py",
        code_root / "scripts" / "analyze_stage_0_1_sumo.py",
    )
    result: dict[str, str] = {}
    for path in files:
        if not path.is_file():
            raise FormalArtifactError(f"Stage 0.1 implementation file is missing: {path}")
        try:
...
def _run_cell(
    *, work: Path, run_dir: Path, cell_id: str, seed: int, repo_root: Path, command: list[str]
    ...
    try:
        for sequence in range(160):
            ...
            for vehicle_id in ids:
                ...
                row = {
                    "run_id": run_id, "experiment_id": "stage_0_1", "scenario_id": "dual_corridor_sumo_minimal",
                ...
                if vehicle_id == "target_0":
                    ...
                    risk_rows.append({
                        "run_id": run_id, "cell_id": cell_id, "target_vehicle_id": vehicle_id,
                        "window_id": "stage_0_1_validation_window", "window_start_s": 0.0, "window_end_s": 32.0,
                        "metric": "ttc_burden_validation", "value": max(2.0 - ttc, 0.0) if ttc != float("inf") else 0.0,
                        "tau_p_lower_s": float("inf"), "attributable": False,
                        "inclusion_status": "stage_0_1_validation", "inclusion_reason": "not_calibrated_for_science",
                    })
    ...
    config = {
        "schema_version": "stage_0_1.sumo.v1", "experiment_id": "stage_0_1", "theory_version": "nc-prereg-v1.0",
        "protocol_version": "stage_0_1.sumo-minimal.v1", "scenario_id": "dual_corridor_sumo_minimal", "pair_id": run_dir.parent.name,
    ...
    audit = {
        "schema_version": "stage_0_1.audit.v1", "experiment_id": "stage_0_1", "run_id": run_id,
        "scientific_claim_eligible": False, "theory_schema_pass": True, "protocol_audit": protocol_audit,
        "physical_path_audit": path_audit,
        "note": "Real SUMO Stage 0.1 infrastructure run; validation only, not a scientific effect result.",
    }
    ...
    (run_dir / "run.log").write_text(f"run_kind=stage_0_1_real_sumo\ncell_id={cell_id}\nscientific_claim_eligible=false\nexit_code=0\n", encoding="utf-8", newline="\n")
    rows = validate_theory_artifact_schema(run_dir)
    build_run_manifest(run_dir, metadata={"experiment_id": "stage_0_1", "protocol_version": "stage_0_1.sumo-minimal.v1", "run_id": run_id, "cell_id": cell_id, "scientific_claim_eligible": False, "row_counts": rows})
...
def run_real_sumo_stage_0_1(output_root: Path, *, run_id: str, seed: int, repo_root: Path, command: list[str]) -> Path:
    """Run one deterministic four-cell Stage 0.1 package against real SUMO."""

    output_root = Path(output_root).resolve()
    package = output_root / run_id
    if package.exists():
        raise FormalArtifactError(f"Refusing to overwrite an existing Stage 0.1 package: {package}")
    ...
    with tempfile.TemporaryDirectory(prefix="aad_stage_0_1_sumo_") as temp:
        work = Path(temp)
        _write_network(work)
    ...
    report = {"schema_version": "stage_0_1.sumo-report.v1", "experiment_id": "stage_0_1", "run_id": run_id, "scientific_claim_eligible": False, "four_cell_audit": validate_four_cell_configs(configs), "pre_treatment_audit": pre, "state_isolation_audit": isolation, "time_alignment_audit": time_audit, "vehicle_lifecycle_audit": lifecycle, "protocol_audits": protocols, "physical_path_audit": path, "note": "Real SUMO four-cell package for Stage 0.1 infrastructure validation only."}
    _write_json(package / "stage_0_1_report.json", report)
    return package


def analyze_real_sumo_stage_0_1(package: Path) -> dict[str, Any]:
    """Recompute Stage 0.1 audit facts only from sealed files, never runner state."""
