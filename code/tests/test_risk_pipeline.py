...
from riskprop.feasibility_data import generate_merge_feasibility_emissions
from riskprop.pipeline import run_pipeline
from riskprop.propagation import (
...
def test_runner_script_executes_pipeline(tmp_path: Path):
    ...
    result = subprocess.run(
        [sys.executable, str(project_root / "scripts" / "run_pipeline.py"), "--input", str(input_csv), "--output", str(output_dir)],
        cwd=project_root,
        check=False,
        capture_output=True,
        text=True,
