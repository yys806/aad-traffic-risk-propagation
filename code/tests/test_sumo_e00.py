...
from riskprop.sumo_e00 import (
    analyze_physical_path_from_archived_tables,
    analyze_real_sumo_e00,
    run_real_sumo_e00,
)
...
def test_real_sumo_e00_seals_nonempty_four_cell_package_and_read_only_audit(tmp_path: Path):
    package = run_real_sumo_e00(
        tmp_path,
        run_id="sumo_e2e",
        seed=2026080701,
        repo_root=Path(__file__).resolve().parents[2],
        command=["pytest", "test_real_sumo_e00"],
    )

    result = analyze_real_sumo_e00(package)
    ...
    assert result["cell_summaries"]["s0c1"]["risk_inclusion_statuses"] == ["e00_validation"]
