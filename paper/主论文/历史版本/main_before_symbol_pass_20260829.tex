...
\revBlue{本文研究局部交通场景中的信息介导风险传播与因果识别机制，而不是把整个交通系统作为研究对象。车辆运动和物理交互构成背景机制，显式消息构成可干预的信息通道。设 $\mathcal V=\{1,\ldots,N\}$ 为车辆集合，$\mathbf S_t=\{\mathbf s_{i,t}:i\in\mathcal V\}$ 为全体车辆在 $t$ 时刻的状态，$\mathbf U_t=\{\mathbf u_{i,t}^{\mathsf{act}}:i\in\mathcal V\}$ 为实际控制输入，$\mathcal G^{\mathsf P}(t)$ 和 $\mathcal G^{\mathsf I}(t)$ 分别表示物理交通图和显式信息图，$\mathcal M_t$ 表示截至时刻 $t$ 已生成且尚在协议生命周期内的消息集合。共同系统状态写为}
\begin{equation}
\color{advisorBlue}
\Sigma_t=\left(\mathbf S_t,\mathbf U_t,\mathcal G^{\mathsf P}(t),\mathcal G^{\mathsf I}(t),\mathcal M_t\right),
\end{equation}
...
\revBlue{车辆驾驶模式记为 $\mu_i\in\{\mathsf{AV},\mathsf{HV}\}$；该标签只表示驾驶模式，不改变车辆索引。对每个时刻 $t$，$\mathcal{N}_i^{\mathsf P}(t)$ 表示能够通过跟驰、合流或冲突直接影响车辆 $i$ 的物理邻居，$\mathcal{N}_i^{\mathsf I}(t)$ 表示通信协议允许向车辆 $i$ 提供消息的来源集合。由有向序对 $(i,j)$ 表示从源车辆 $i$ 指向目标车辆 $j$，物理图和信息图定义为}
\begin{equation}
\color{advisorBlue}
\begin{aligned}
\mathcal{E}^{\mathsf P}(t)&=\{(i,j):i\in\mathcal{N}_j^{\mathsf P}(t),\ i\neq j\},\\
\mathcal{E}^{\mathsf I}(t)&=\{(i,j):i\in\mathcal{N}_j^{\mathsf I}(t),\ i\neq j\},\\
...
\begin{equation}
\color{advisorBlue}
\mathbf s_{i,t}=\bigl(\mathbf p_{i,t},v_{i,t},a_{i,t},\ell_{i,t}\bigr),
\end{equation}
其中 $\mathbf p_{i,t}$ 为二维位置，$v_{i,t}$ 为速度，$a_{i,t}$ 为实际纵向加速度，$\ell_{i,t}$ 为车道和道路边等交通环境状态。$\mathbf u_{i,t}^{\mathsf{cmd}}$ 表示控制器请求的纵向或横向命令，$\mathbf u_{i,t}^{\mathsf{act}}$ 表示该命令经过车辆执行器和安全约束后实际施加的控制输入，$\boldsymbol\xi_{i,t}$ 表示外生随机输入。离散仿真时钟上的状态更新写为
\begin{equation}
\color{advisorBlue}
\mathbf s_{i,t+1}=f_i^{\mathsf{dyn}}\!\left(\mathbf s_{i,t},\mathbf u_{i,t}^{\mathsf{act}},\boldsymbol\xi_{i,t};\mathcal G^{\mathsf P}(t)\right).
\end{equation}
函数 $f_i^{\mathsf{dyn}}$ 包含车辆运动学、道路边界和物理邻居作用；其具体跟驰、制动、换道和合流规则由实验配置指定，不在本节预设某一种控制器。所有车辆状态、控制输入和消息事件使用同一仿真时钟记录。

前述消息集合 $\mathcal M_t$ 中的每条消息由唯一标识 $\mathsf{id}$、来源车辆 $i$、目标车辆 $j$、消息载荷和协议时间戳组成，记为 $m_{\mathsf{id},i,j,t}$。协议允许发往目标车辆 $j$ 的候选消息集合为
\begin{equation}
\color{advisorBlue}
\mathcal M_{j,t}^{\mathsf{cand}}=\left\{m_{\mathsf{id},i,j,t}\in \mathcal M_t:
...
\revGreen{设 $S\in\{0,1\}$ 表示指定源风险是否存在，$C\in\{0,1\}$ 表示指定消息通道是否开放。对完整四格配对单元 $\omega$，令 $R_{j,\omega}(s,c)$ 表示目标车辆 $j\in\mathcal J^{\mathsf T}_{\omega}$ 在处理条件 $(S=s,C=c)$ 下的窗口风险暴露，即该车在结果窗口内按预先冻结指标和有效观测掩码计算的风险强度；目标区域总体风险为}
\begin{equation}
...
\revBlue{消息处理状态沿用状态字母 $S$，记为 $S_{\mathsf{id},j}^{\mathsf{del}}(t)$、$S_{\mathsf{id},j}^{\mathsf{chk}}(t)$ 和 $S_{\mathsf{id},j}^{\mathsf{adopt}}(t)$，分别表示消息 $\mathsf{id}$ 对目标车辆 $j$ 已交付、已通过联合校验和已被控制器采用；三者均属于 $\{0,1\}$。令 $t_0$ 为配对试验的共同触发时刻，$t_{\mathsf{id}}^{\mathsf{gen}}$、$t_{\mathsf{id}}^{\mathsf{send}}$、$t_{\mathsf{id},j}^{\mathsf{del}}$ 和 $t_{\mathsf{id},j}^{\mathsf{adopt}}$ 分别表示消息生成、发送、交付和采用时刻。}

为排除数值噪声，令 $\varepsilon^{\mathsf{input}}>0$ 和 $\varepsilon^{\mathsf{state}}>0$ 分别为实际输入差异和状态差异的判定容差。从共同触发时刻 $t_0$ 开始，$t_j^{\mathsf{input}}$ 表示“存在源风险且开放消息”和“存在相同源风险但屏蔽消息”两种条件下，目标车辆的实际施加输入 $\mathbf u_{j,t}^{\mathsf{act}}$ 首次出现明显差异的时刻；$t_j^{\mathsf{state}}$ 表示在两种条件均屏蔽消息时，源风险存在与否首次使目标状态 $\mathbf s_{j,t}$ 出现明显差异的时刻。物理影响到达时刻 $\tau_j^{\mathsf P}$ 沿用前文的时序物理路径审计定义，不用单个状态差异替代。前者用于信息先行审计，后者只用于校核物理传播估计。两个可观测分化时刻定义为
\begin{equation}
\begin{aligned}
t_j^{\mathsf{input}}&=\inf\left\{t\geq t_0:\left\|\mathbf u_{j,t}^{\mathsf{act}}(1,1)-\mathbf u_{j,t}^{\mathsf{act}}(1,0)\right\|>\varepsilon^{\mathsf{input}}\right\},\\
t_j^{\mathsf{state}}&=\inf\left\{t\geq t_0:\left\|\mathbf s_{j,t}(1,0)-\mathbf s_{j,t}(0,0)\right\|>\varepsilon^{\mathsf{state}}\right\},
\end{aligned}
\end{equation}
...
下列约束中的 $\mathsf{id}$ 表示满足约束~\eqref{eq:P1-d} 的一条已采用消息。
\begin{subequations}
\begin{align}
\mathrm{s.t.}\;&\mathcal R^{\mathsf S}\cap\mathcal R^{\mathsf T}=\varnothing;
\label{eq:P1-a}\\
&\operatorname{Reach}_{\mathcal G^{\mathsf P}}\!(i,j;[t_0,t^{\mathsf{win}}+H])=0;
\label{eq:P1-b}\\
...
\label{eq:P1-c}\\
&\begin{aligned}
\exists \mathsf{id}\in\mathcal M_{\omega}^{\mathsf{src}}:\quad
S_{\mathsf{id},j}^{\mathsf{adopt}}(t_{\mathsf{id},j}^{\mathsf{adopt}})&=1;
\end{aligned}
\label{eq:P1-d}\\
&S_{\mathsf{id},j}^{\mathsf{adopt}}(t)=1\Rightarrow S_{\mathsf{id},j}^{\mathsf{del}}(t)=1\quad\forall t;
\label{eq:P1-e}\\
&S_{\mathsf{id},j}^{\mathsf{adopt}}(t)=1\Rightarrow S_{\mathsf{id},j}^{\mathsf{chk}}(t)=1\quad\forall t;
\label{eq:P1-f}\\
&t_0\leq t_{\mathsf{id}}^{\mathsf{gen}}\leq t_{\mathsf{id}}^{\mathsf{send}};
\label{eq:P1-g}\\
&t_{\mathsf{id}}^{\mathsf{send}}\leq t_{\mathsf{id},j}^{\mathsf{del}};
\label{eq:P1-h}\\
&t_{\mathsf{id},j}^{\mathsf{del}}\leq t_{\mathsf{id},j}^{\mathsf{adopt}};
\label{eq:P1-i}\\
&t_{\mathsf{id},j}^{\mathsf{adopt}}\leq t_j^{\mathsf{input}};
\label{eq:P1-j}\\
&t_j^{\mathsf u}<\tau_j^{\mathsf P};
\label{eq:P1-k}\\
&t_{\mathsf{id},j}^{\mathsf{adopt}}\leq t^{\mathsf{win}};
\label{eq:P1-l}\\
...
Stage II 沿用消息状态 $S_{\mathsf{id},j}^{\mathsf{del}}(t)$、$S_{\mathsf{id},j}^{\mathsf{chk}}(t)$ 和 $S_{\mathsf{id},j}^{\mathsf{adopt}}(t)$。消息 $\mathsf{id}$ 在时刻 $t$ 的年龄为 $t-t_{\mathsf{id}}^{\mathsf{gen}}$，$T^{\mathsf{fresh}}>0$ 为最大允许年龄。令 $\widetilde{\mathcal M}_{j,t}\subseteq\mathcal M_{j,t}^{\mathsf{cand}}$ 为已经交付、通过联合校验且未过期的消息集合。采用策略 $\pi^{\mathsf{adopt}}$ 为每条合格消息给出二元采用决定；被采用的消息组成 $\mathcal M_{j,t}^{\mathsf{adopt}}$，再经冻结的消息聚合器形成控制信息 $\mathbf z_{j,t}$。动作策略 $\pi^{\mathsf u}$ 根据本地状态和 $\mathbf z_{j,t}$ 生成请求命令 $\mathbf u_{j,t}^{\mathsf{cmd}}$，安全执行映射 $g_j^{\mathsf{safe}}$ 将其转化为实际输入 $\mathbf u_{j,t}^{\mathsf{act}}$。联合策略记为 $\pi=(\pi^{\mathsf{adopt}},\pi^{\mathsf u})$。
具体地，合格消息集合定义为
\begin{equation*}
\begin{aligned}
\widetilde{\mathcal M}_{j,t}=\{m_{\mathsf{id},i,j,t}\in \mathcal M_{j,t}^{\mathsf{cand}}:
&S_{\mathsf{id},j}^{\mathsf{del}}(t)=1,\ S_{\mathsf{id},j}^{\mathsf{chk}}(t)=1,\\
&0\leq t-t_{\mathsf{id}}^{\mathsf{gen}}\leq T^{\mathsf{fresh}}\}.
\end{aligned}
\end{equation*}
...
\begin{subequations}
\begin{align}
\mathrm{s.t.}\;&\mathbf s_{j,t+1}=f_j^{\mathsf{dyn}}(\mathbf s_{j,t},\mathbf u_{j,t}^{\mathsf{act}},\boldsymbol\xi_{j,t};
\mathcal G^{\mathsf P}(t));
\label{eq:P2-a}\\
&\mathbf u_{j,t}^{\mathsf{cmd}}=\pi^{\mathsf u}(\mathbf s_{j,t},\mathbf z_{j,t})
\quad\forall t;
\label{eq:P2-b}\\
&\begin{aligned}
S_{\mathsf{id},j}^{\mathsf{adopt}}(t)&=\pi_{\mathsf{id}}^{\mathsf{adopt}}(\mathbf s_{j,t},m_{\mathsf{id},i,j,t})\in\{0,1\},\\
&\hspace{-2em}\forall m_{\mathsf{id},i,j,t}\in\widetilde{\mathcal M}_{j,t};
\end{aligned}
\label{eq:P2-c}\\
&\begin{aligned}
\mathcal M_{j,t}^{\mathsf{adopt}}=\{m_{\mathsf{id},i,j,t}\in\widetilde{\mathcal M}_{j,t}:\\
S_{\mathsf{id},j}^{\mathsf{adopt}}(t)=1\};
\end{aligned}
\label{eq:P2-d}\\
&\begin{aligned}
\mathbf z_{j,t}&=\operatorname{Agg}^{\mathsf{msg}}(\mathcal M_{j,t}^{\mathsf{adopt}}),\\
\operatorname{Agg}^{\mathsf{msg}}(\varnothing)&=\mathbf 0;
\end{aligned}
\label{eq:P2-e}\\
&S_{\mathsf{id},j}^{\mathsf{adopt}}(t)=1\Rightarrow S_{\mathsf{id},j}^{\mathsf{del}}(t)=1\quad\forall \mathsf{id},t;
\label{eq:P2-f}\\
&S_{\mathsf{id},j}^{\mathsf{adopt}}(t)=1\Rightarrow S_{\mathsf{id},j}^{\mathsf{chk}}(t)=1\quad\forall \mathsf{id},t;
\label{eq:P2-g}\\
&S_{\mathsf{id},j}^{\mathsf{adopt}}(t)=1\Rightarrow
0\leq t-t_{\mathsf{id}}^{\mathsf{gen}}\leq T^{\mathsf{fresh}}\quad\forall \mathsf{id},t;
\label{eq:P2-h}\\
&\begin{aligned}
\mathbf u_{j,t}^{\mathsf{act}}&=g_j^{\mathsf{safe}}(\mathbf s_{j,t},\mathbf u_{j,t}^{\mathsf{cmd}}),\\
\mathbf u_{j,t}^{\mathsf{act}}&\in\mathcal U_j\quad\forall t;
\end{aligned}
\label{eq:P2-i}\\
&\mathbf s_{j,t}\in\mathcal X_j^{\mathsf{safe}}\quad\forall t;
\label{eq:P2-j}\\
...
\end{align}
\end{subequations}
约束~\eqref{eq:P2-a} 使用实际输入更新车辆状态，约束~\eqref{eq:P2-b} 定义动作策略的请求命令。约束~\eqref{eq:P2-c}--\eqref{eq:P2-e} 依次定义逐消息采用、已采用消息集合和控制信息聚合，因此 $S_{\mathsf{id},j}^{\mathsf{adopt}}(t)$、$\mathbf z_{j,t}$ 与控制命令具有可追溯的数学关系。约束~\eqref{eq:P2-f}--\eqref{eq:P2-h} 要求被采用的消息已经交付、通过校验且未过期。约束~\eqref{eq:P2-i} 将请求命令经过安全执行映射转化为实际输入，约束~\eqref{eq:P2-j} 要求全过程处于安全状态。约束~\eqref{eq:P2-k}--\eqref{eq:P2-m} 分别限制动作与舒适性、通行效率和尾部风险，防止策略通过急刹、停车或降低通行效率换取表面的平均风险改善。
